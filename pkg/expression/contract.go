...
type VariableEvaluator interface {
	// EvaluateWritePath computes a location without exposing the caller's value.
	EvaluateWritePath(ctx context.Context, expression string, vars map[string]any) ([]any, error)
