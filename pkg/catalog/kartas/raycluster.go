					...
					StatusDefinition: &v1alpha1.StatusDefinition{
						...
						StatusMappings: v1alpha1.StatusMappings{
							...
							Initializing: []v1alpha1.StatusMatcher{{ByExpression: &v1alpha1.ExpressionMatcher{
								Expression:     `object.?spec.?suspend.orValue(false) != true && variables.statusState != "ready" && variables.statusState != "failed"`,
								ExpectedResult: "true",
							}}},
							Suspended: []v1alpha1.StatusMatcher{{ByExpression: &v1alpha1.ExpressionMatcher{
								Expression:     `object.?spec.?suspend.orValue(false) == true && (variables.statusState == "suspended" || variables.statusState == "")`,
								ExpectedResult: "true",
							}}},
						},
					},
				...
				ChildComponents: []v1alpha1.ComponentDefinition{
					{
						...
						SpecDefinition: &v1alpha1.SpecDefinition{
							PodTemplateSpec: &v1alpha1.ValueAccessor{Expression: `object[?"spec"][?"headGroupSpec"][?"template"].orValue(null)`, Patches: []v1alpha1.PatchEntry{{PatchType: v1alpha1.PatchTypeMergePatch, Expression: `{"spec": {"headGroupSpec": {"template": value}}}`}}, PatchStrategy: v1alpha1.PatchStrategyReplace},
						},
					...
					},
					{
						...
						OwnerRef: ptr.To("raycluster"),
						SpecDefinition: &v1alpha1.SpecDefinition{
							PodTemplateSpec: &v1alpha1.ValueAccessor{Expression: `([dyn(object[?"spec"][?"workerGroupSpecs"].orValue(null))].filter(v, type(v) == list) + [[]])[0].map(x, x[?"template"].orValue(null))`, Patches: []v1alpha1.PatchEntry{{PatchType: v1alpha1.PatchTypeJSONPatch, Expression: `[{"op": "add", "path": "/spec/workerGroupSpecs/" + string(index) + "/template", "value": value}]`}}},
						},
						ScaleDefinition: &v1alpha1.ScaleDefinition{
							Replicas:    &v1alpha1.ValueAccessor{Expression: `([dyn(object[?"spec"][?"workerGroupSpecs"].orValue(null))].filter(v, type(v) == list) + [[]])[0].map(x, x[?"replicas"].orValue(null))`},
							MinReplicas: &v1alpha1.ValueAccessor{Expression: `([dyn(object[?"spec"][?"workerGroupSpecs"].orValue(null))].filter(v, type(v) == list) + [[]])[0].map(x, x[?"minReplicas"].orValue(null))`},
							MaxReplicas: &v1alpha1.ValueAccessor{Expression: `([dyn(object[?"spec"][?"workerGroupSpecs"].orValue(null))].filter(v, type(v) == list) + [[]])[0].map(x, x[?"maxReplicas"].orValue(null))`},
						},
						InstanceIds: &v1alpha1.ValueAccessor{Expression: `([dyn(object[?"spec"][?"workerGroupSpecs"].orValue(null))].filter(v, type(v) == list) + [[]])[0].map(x, x[?"groupName"].orValue(null))`},
						PodSelector: &v1alpha1.PodSelector{
							ComponentTypeSelector: &v1alpha1.ComponentTypeSelector{
								Expression: `object[?"metadata"][?"labels"][?"ray.io/node-type"].orValue(null)`,
								Value:      ptr.To("worker"),
