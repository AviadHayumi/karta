...
func TestApplyConstructedPatchAddParents(t *testing.T) {
	...
	out, err := applyConstructedPatch(live, ops, PatchTypeJSONPatch)
	if err != nil {
	...
	if _, err := applyConstructedPatch(live, badIndex, PatchTypeJSONPatch); err == nil {
	...
	if _, err := applyConstructedPatch(wholeElement, elementAdd, PatchTypeJSONPatch); err == nil {
	...
	if _, err := applyConstructedPatch(missingList, numericParent, PatchTypeJSONPatch); err == nil {
	...
	out2, err := applyConstructedPatch(escaped, ops2, PatchTypeJSONPatch)
	if err != nil {
