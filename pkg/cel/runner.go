...
// runner evaluates reads and write destinations with CEL. A write expression
// returns a JSON Pointer string, not an assignable CEL expression. The SDK
// applies the caller's value at that pointer and publishes the updated document.
type runner struct {
	evaluator *evaluator
...
// EvaluateWithVariables evaluates a CEL expression with additional bindings.
// The result is one entry holding the whole value; a list is never spread.
// Write destinations use EvaluateWritePath to prohibit the value binding.
...
// EvaluateWritePath rejects value-dependent locations, including dependencies
// through named variables. CEL's compiler distinguishes identifiers from map
// keys, string literals, and locally bound comprehension variables.
func (r *runner) EvaluateWritePath(ctx context.Context, expression string, vars map[string]any) ([]any, error) {
	checks := []string{expression}
	needed := neededVariables(r.variables, checks)
	for _, variable := range r.variables {
		if needed == nil || needed[variable.Name] {
			checks = append(checks, variable.Expression)
		}
	}
	for _, check := range checks {
		if _, issues := r.evaluator.pathEnv.Compile(check); issues != nil && issues.Err() != nil {
			return nil, fmt.Errorf("compile write path (value is unavailable): %w", issues.Err())
		}
	}
	bindings := map[string]any{"instance": vars["instance"], "index": vars["index"]}
	return r.EvaluateWithVariables(ctx, expression, bindings)
}
...
// Assign publishes an SDK-prepared document. CEL itself does not mutate objects;
// the SDK resolves the destination and applies its policy before this call.
func (r *runner) Assign(ctx context.Context, expression string, value any) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if strings.TrimSpace(expression) != "." {
		return fmt.Errorf("cannot assign to %q: the CEL runner only publishes complete documents", expression)
	}
